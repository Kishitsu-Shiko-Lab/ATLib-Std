package ATLib::Std::Exception::InvalidCast;
use Mouse;
...
ATLib::Std::Exception::InvalidCast - 無効なキャストを表す構造化例外
...
ATLib::Exception::InvalidCast は、ATLib::Stdで提供される L<< Mouse >> で実装された無効なキャストを表す構造化例外型です。

=head1 コンストラクタ

=head2 C<< $instance = ATLib::Std::Exception::InvalidCast->new({ message => $message }); >>
